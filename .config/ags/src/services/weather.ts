...
export default class Vpn extends GObject.Object {
  ...
  public async fetch_location() {
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?city=${this.#city}&limit=1&format=json`,
      );
  ...
  public async fetch_weather() {
    try {
      const { latitude, longitude, city, country } =
        await this.fetch_location();
      const response = await fetch(
  ...
  public poll() {
    interval(INTERVAL, async () => {
      this.#weather = await this.fetch_weather();
      this.notify('weather');
    });
  }
  ...
  constructor() {
    super();
    this.fetch_location = this.fetch_location.bind(this);
